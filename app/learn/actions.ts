...
export async function migrateLessonExercises() {
  const supabase = await createClient()
  ...
  for (const lesson of lessons) {
    ...
    // Pattern 1: Match "Number. English [Arrow] Swedish"
    // Handles various arrows: →, ->, — 
    const arrowPattern = /^(\d+\.\s+.*?)\s*(?:→|->|—|--|→)\s*(.*)$/gm
    
    // Pattern 2: Match "Number. (Parentheses Question) [Arrow] Swedish (Translation)"
    ...
    if (hasChanges) {
      ...
      if (error) console.error(`Error updating ${lesson.slug}:`, error)
    } else {
      console.log(`No changes needed for ${lesson.slug}`)
    }
  }
